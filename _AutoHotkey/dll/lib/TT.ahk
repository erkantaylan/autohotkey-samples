...
TT(options:="",text:="",title:=""){
  ...
  ; Objects
  static _:={},base:={Color:"TT_Color",Show:"TT_Show",Hide:"TTM_Trackactivate",Close:"TT_Close",Add:"TT_Add",AddTool:"TTM_AddTool"
  ...
  If (options=-0x8000000000000000) 
    Return _
  else if (Type(options+0)="Integer")
    Parent:=options
  else If (options){
...
TT_Remove(T:=""){
  static _:=TT(-0x8000000000000000) ;Get main object that holds all ToolTips
  If !T
    Loop % _.MaxIndex()
    {
      T:=_[A_Index]
...
TT_OnMessage(wParam,lParam,msg,hwnd){
  ...
  static TTN_FIRST:=0xfffffdf8, _:=TT(-0x8000000000000000) ;Get main object that holds all ToolTips
...
TT_GetIcon(File:="",Icon_:=1){
  ...
  static hIcon:={},AW:=A_IsUnicode?"W":"A",pToken:=0
   ,temp1:=DllCall( "LoadLibrary", "Str","gdiplus","PTR"),temp2:=VarSetCapacity(si, 16, 0) (si := Chr(1)) DllCall("gdiplus\GdiplusStartup", "PTR*",pToken, "PTR",&si, "PTR",0)
	...
	static sfi:=Struct(_SHFILEINFO),sfi_size:=sizeof(_SHFILEINFO),SmallIconSize:=SysGet(49)
	If !File {
